...
function clickHandler(event) {
	...
	function onMessage(event) {
		...
		user_img.innerText = JSON.parse(event.data).username
		console.log(user_img.innerText)
		if(user_img.innerText == 'alarm') return
		...
		fetch(cpath + '/getNickNameUser/' + user_img.innerText)
		.then(resp => resp.json())
		.then(json => {
			console.log(json)
			let tag = ''
			if(json.nick_name != username) {
				tag += '<a href="' + cpath + '/users/viewDetail/' + json.idx + '">'
				tag += '<img src="' + cpath + '/resources/img/' + json.user_img + '">'
				user_img.innerHTML = tag
				wrap.appendChild(user_img)
			}
			
			wrap.appendChild(message)
			wrap.classList.add(JSON.parse(event.data).username == username ? 'self' : 'other')
